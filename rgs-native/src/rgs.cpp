...
static constexpr const char* APP_VERSION = "5.3.0-native-cpp-external-dedupe";
...
struct Args {
    ...
    size_t dedupe_chunk_bytes = 512ULL * 1024ULL * 1024ULL;
    size_t merge_fan_in = 256;
    std::string temp_dir = "";
    unsigned threads = 0;
...
static size_t parse_memory_size_or_mb(std::string text) {
    text = trim_copy(text);
    if (text.empty()) return 0;
    bool has_suffix = !std::isdigit(static_cast<unsigned char>(text.back()));
    size_t value = parse_size(text);
    if (!has_suffix && value > 0) value *= 1024ull * 1024ull;
    return value;
}
...
static size_t approx_record_memory(const DedupeRecord& r) {
    return sizeof(DedupeRecord) + r.a.size() + r.b.size() + r.c.size() + r.file.size() + r.source_line.size();
}

static bool write_u64_le_stream(std::ofstream& out, uint64_t value) {
    char buf[8];
    for (int i = 0; i < 8; ++i) buf[i] = static_cast<char>((value >> (i * 8)) & 0xffu);
    out.write(buf, sizeof(buf));
    return static_cast<bool>(out);
}

static bool read_u64_le_stream(std::ifstream& in, uint64_t& value) {
    char buf[8];
    if (!in.read(buf, sizeof(buf))) return false;
    value = 0;
    for (int i = 0; i < 8; ++i) value |= (static_cast<uint64_t>(static_cast<unsigned char>(buf[i])) << (i * 8));
    return true;
}

static bool write_string_stream(std::ofstream& out, const std::string& value) {
    if (!write_u64_le_stream(out, static_cast<uint64_t>(value.size()))) return false;
    if (!value.empty()) out.write(value.data(), static_cast<std::streamsize>(value.size()));
    return static_cast<bool>(out);
}

static bool read_string_stream(std::ifstream& in, std::string& value) {
    uint64_t n = 0;
    if (!read_u64_le_stream(in, n)) return false;
    // Chunks are generated by this process. This guard avoids runaway allocation if
    // a temp file is corrupted or manually replaced.
    if (n > (1ULL << 34)) return false;
    value.assign(static_cast<size_t>(n), '\0');
    if (n > 0 && !in.read(value.data(), static_cast<std::streamsize>(n))) return false;
    return true;
}

static bool write_record_stream(std::ofstream& out, const DedupeRecord& r, bool full_columns) {
    if (!write_string_stream(out, r.a)) return false;
    if (!write_string_stream(out, r.b)) return false;
    if (!write_string_stream(out, r.c)) return false;
    if (full_columns) {
        if (!write_string_stream(out, r.file)) return false;
        if (!write_u64_le_stream(out, r.line)) return false;
        if (!write_string_stream(out, r.source_line)) return false;
    }
    return static_cast<bool>(out);
}

static bool read_record_stream(std::ifstream& in, DedupeRecord& r, bool full_columns) {
    DedupeRecord tmp;
    if (!read_string_stream(in, tmp.a)) return false;
    if (!read_string_stream(in, tmp.b)) return false;
    if (!read_string_stream(in, tmp.c)) return false;
    if (full_columns) {
        if (!read_string_stream(in, tmp.file)) return false;
        if (!read_u64_le_stream(in, tmp.line)) return false;
        if (!read_string_stream(in, tmp.source_line)) return false;
    }
    r = std::move(tmp);
    return true;
}
...
public:
    ChunkReader() = default;
    bool open(const fs::path& path, bool full_columns) {
        full_columns_ = full_columns;
        in_.open(path, std::ios::binary);
        return static_cast<bool>(in_);
    }
    bool next(DedupeRecord& out) {
        if (!in_) return false;
        return read_record_stream(in_, out, full_columns_);
    }
private:
    std::ifstream in_;
    bool full_columns_ = false;
};
...
struct MergeNodeGreater {
    bool operator()(const MergeNode& x, const MergeNode& y) const {
        return dedupe_record_less(y.rec, x.rec);
    }
};
...
public:
    ...
    ~DirectoryDeduper() { cleanup_temp_dir(); }

    bool run(Stats& stats, std::string* error_message = nullptr) {
        start_ = std::chrono::steady_clock::now();
        if (args_.threads == 0) args_.threads = 1;  // Standalone dedupe defaults to lowest memory.
        if (!validate(error_message)) return false;
        if (!prepare_temp_dir(error_message)) return false;
        ...
        const unsigned nthreads = std::min<unsigned>(args_.threads, static_cast<unsigned>(files.size()));
        const size_t total_cap = std::max<size_t>(args_.dedupe_chunk_bytes, 16ULL * 1024ULL * 1024ULL);
        const size_t per_thread_cap = std::max<size_t>(8ULL * 1024ULL * 1024ULL, total_cap / std::max<unsigned>(1, nthreads));

        std::atomic<size_t> index{0};
        std::vector<std::thread> workers;
        for (unsigned i = 0; i < nthreads; ++i) {
            workers.emplace_back([&, per_thread_cap]() {
                std::vector<DedupeRecord> local_rows;
                local_rows.reserve(8192);
                size_t local_bytes = 0;
                while (true) {
                    if (fatal_.load(std::memory_order_relaxed)) break;
                    size_t pos = index.fetch_add(1, std::memory_order_relaxed);
                    if (pos >= files.size()) break;
                    scan_file(files[pos], stats, local_rows, local_bytes, per_thread_cap, error_message);
                }
                if (!fatal_.load(std::memory_order_relaxed)) flush_chunk(local_rows, local_bytes, stats, error_message);
            });
        }
        for (auto& t : workers) t.join();
        if (fatal_.load(std::memory_order_relaxed)) return false;
        ...
        if (chunk_paths_.empty()) {
            ...
            if (!writer.close_and_commit(error_message)) return false;
            write_summary(stats, files.size(), 0, 0, 0);
            return true;
        }

        size_t merge_passes = 0;
        while (chunk_paths_.size() > std::max<size_t>(2, args_.merge_fan_in)) {
            if (!merge_pass_to_chunks(stats, error_message)) return false;
            ++merge_passes;
        }

        uint64_t unique_total = 0;
        if (!merge_final_to_output(stats, unique_total, error_message)) return false;
        write_summary(stats, files.size(), unique_total, chunk_paths_.size(), merge_passes);
        return true;
    }
...
private:
    bool validate(std::string* err) {
        ...
        args_.columns = cols;
        args_.parse_mode = pm;
        if (args_.merge_fan_in < 2) args_.merge_fan_in = 2;
        if (args_.merge_fan_in > 1024) args_.merge_fan_in = 1024;
        if (args_.dedupe_chunk_bytes < 16ULL * 1024ULL * 1024ULL) args_.dedupe_chunk_bytes = 16ULL * 1024ULL * 1024ULL;
        return true;
    }

    bool prepare_temp_dir(std::string* error_message) {
        fs::path base;
        if (!args_.temp_dir.empty()) base = fs::path(args_.temp_dir);
        else {
            fs::path out(args_.output);
            base = out.parent_path().empty() ? fs::temp_directory_path() : out.parent_path();
        }
        auto tick = std::chrono::steady_clock::now().time_since_epoch().count();
        std::ostringstream name;
        name << "rgs_dedupe_tmp_" << current_process_id_number() << "_" << tick;
        temp_root_ = base / name.str();
        std::error_code ec;
        fs::create_directories(temp_root_, ec);
        if (ec) {
            if (error_message) *error_message = "Cannot create temp directory: " + temp_root_.generic_string() + ": " + ec.message();
            return false;
        }
        return true;
    }
    ...
    fs::path new_chunk_path() {
        uint64_t id = chunk_counter_.fetch_add(1, std::memory_order_relaxed);
        std::ostringstream os;
        os << "chunk_" << std::setw(8) << std::setfill('0') << id << ".bin";
        return temp_root_ / os.str();
    }
    ...
    void add_file(const fs::path& p, std::vector<fs::path>& files, Stats& stats) {
        ...
        if (!args_.summary.empty() && same_path_key(p, fs::path(args_.summary))) return;
        if (!temp_root_.empty()) {
            std::string full = normalized_path_key(p);
            std::string tmp = normalized_path_key(temp_root_);
            if (starts_with(full, tmp)) return;
        }
    ...
    void set_fatal(const std::string& msg, std::string* error_message) {
        bool expected = false;
    ...
    void scan_file(const fs::path& path, Stats& stats, std::vector<DedupeRecord>& local_rows, size_t& local_bytes, size_t cap_bytes, std::string* error_message) {
        MappedFile mf;
        if (!mf.open_file(path)) { stats.errors.fetch_add(1, std::memory_order_relaxed); return; }
        stats.files_scanned.fetch_add(1, std::memory_order_relaxed);
        stats.bytes_scanned.fetch_add(static_cast<uint64_t>(mf.size()), std::memory_order_relaxed);
        ...
        std::string file_string;
        if (args_.columns == "full") file_string = path.generic_string();
        while (pos < size && !fatal_.load(std::memory_order_relaxed)) {
            const char* start = base + pos;
            const void* nlptr = std::memchr(start, '\n', size - pos);
            size_t len;
            if (nlptr) len = static_cast<const char*>(nlptr) - start;
            ...
            if (parsed) {
                ...
                if (!normalized.a.empty() && !normalized.b.empty()) {
                    ...
                    local_bytes += approx_record_memory(record);
                    local_rows.emplace_back(std::move(record));
                    stats.parsed_triples.fetch_add(1, std::memory_order_relaxed);
                    stats.field_hits.fetch_add(1, std::memory_order_relaxed);
                    if (local_bytes >= cap_bytes) {
                        flush_chunk(local_rows, local_bytes, stats, error_message);
    ...
    void release_local_rows(std::vector<DedupeRecord>& rows, size_t& bytes) {
        std::vector<DedupeRecord>().swap(rows);
        rows.reserve(8192);
        bytes = 0;
    }

    void flush_chunk(std::vector<DedupeRecord>& rows, size_t& bytes, Stats& stats, std::string* error_message) {
        if (rows.empty() || fatal_.load(std::memory_order_relaxed)) return;
        std::sort(rows.begin(), rows.end(), dedupe_record_less);
        fs::path out_path = new_chunk_path();
        std::ofstream out(out_path, std::ios::binary | std::ios::trunc);
        if (!out) { set_fatal("Cannot open temp chunk for writing: " + out_path.generic_string(), error_message); return; }
        bool have_prev = false;
        std::string prev_a, prev_b, prev_c;
        uint64_t unique_in_chunk = 0;
        for (const auto& r : rows) {
            if (have_prev && prev_a == r.a && prev_b == r.b && prev_c == r.c) {
                stats.duplicates.fetch_add(1, std::memory_order_relaxed);
                continue;
            }
            have_prev = true;
            ...
            prev_b = r.b;
            prev_c = r.c;
            if (!write_record_stream(out, r, args_.columns == "full")) {
                set_fatal("Failed while writing temp chunk: " + out_path.generic_string(), error_message);
                return;
            }
            ++unique_in_chunk;
        }
        out.close();
        if (!out) { set_fatal("Failed while closing temp chunk: " + out_path.generic_string(), error_message); return; }
        {
            std::lock_guard<std::mutex> lock(chunk_mutex_);
            chunk_paths_.push_back(out_path);
        }
        if (!args_.quiet) {
            uint64_t idx = chunks_written_.fetch_add(1, std::memory_order_relaxed) + 1;
            if (idx % 50 == 0) {
                std::cerr << "dedupe chunks=" << idx << " parsed=" << stats.parsed_triples.load()
                          << " dup=" << stats.duplicates.load()
                          << " elapsed=" << std::fixed << std::setprecision(2) << elapsed_seconds() << "s\n";
            }
        }
        release_local_rows(rows, bytes);
    }

    bool merge_chunk_group_to_chunk(const std::vector<fs::path>& group, const fs::path& out_path, Stats& stats, std::string* error_message) {
        std::vector<ChunkReader> readers(group.size());
        for (size_t i = 0; i < group.size(); ++i) {
            if (!readers[i].open(group[i], args_.columns == "full")) {
                if (error_message) *error_message = "Cannot open temp chunk for reading: " + group[i].generic_string();
                return false;
            }
        }
        std::priority_queue<MergeNode, std::vector<MergeNode>, MergeNodeGreater> pq;
        for (size_t i = 0; i < readers.size(); ++i) {
            DedupeRecord rec;
            if (readers[i].next(rec)) pq.push(MergeNode{std::move(rec), i});
        }
        std::ofstream out(out_path, std::ios::binary | std::ios::trunc);
        if (!out) { if (error_message) *error_message = "Cannot create merged temp chunk: " + out_path.generic_string(); return false; }
        bool have_prev = false;
        std::string prev_a, prev_b, prev_c;
        while (!pq.empty()) {
            MergeNode node = std::move(const_cast<MergeNode&>(pq.top()));
            pq.pop();
            if (have_prev && prev_a == node.rec.a && prev_b == node.rec.b && prev_c == node.rec.c) {
                stats.duplicates.fetch_add(1, std::memory_order_relaxed);
            } else {
                have_prev = true;
                prev_a = node.rec.a;
                prev_b = node.rec.b;
                prev_c = node.rec.c;
                if (!write_record_stream(out, node.rec, args_.columns == "full")) {
                    if (error_message) *error_message = "Failed while writing merged temp chunk: " + out_path.generic_string();
                    return false;
                }
            }
            DedupeRecord next;
            if (readers[node.reader].next(next)) pq.push(MergeNode{std::move(next), node.reader});
        }
        out.close();
        if (!out) { if (error_message) *error_message = "Failed while closing merged temp chunk: " + out_path.generic_string(); return false; }
        return true;
    }

    bool merge_pass_to_chunks(Stats& stats, std::string* error_message) {
        std::vector<fs::path> old_chunks;
        { std::lock_guard<std::mutex> lock(chunk_mutex_); old_chunks.swap(chunk_paths_); }
        std::vector<fs::path> new_chunks;
        for (size_t i = 0; i < old_chunks.size(); i += args_.merge_fan_in) {
            size_t e = std::min(old_chunks.size(), i + args_.merge_fan_in);
            std::vector<fs::path> group(old_chunks.begin() + static_cast<std::ptrdiff_t>(i), old_chunks.begin() + static_cast<std::ptrdiff_t>(e));
            fs::path out = new_chunk_path();
            if (!merge_chunk_group_to_chunk(group, out, stats, error_message)) return false;
            new_chunks.push_back(out);
            for (const auto& p : group) { std::error_code ec; fs::remove(p, ec); }
        }
        { std::lock_guard<std::mutex> lock(chunk_mutex_); chunk_paths_.swap(new_chunks); }
        if (!args_.quiet) {
            std::cerr << "merge pass complete: chunks=" << chunk_paths_.size()
                      << " dup=" << stats.duplicates.load()
                      << " elapsed=" << std::fixed << std::setprecision(2) << elapsed_seconds() << "s\n";
        }
        return true;
    ...
    bool merge_final_to_output(Stats& stats, uint64_t& unique_total, std::string* error_message) {
        std::vector<ChunkReader> readers(chunk_paths_.size());
        for (size_t i = 0; i < chunk_paths_.size(); ++i) {
            if (!readers[i].open(chunk_paths_[i], args_.columns == "full")) {
                if (error_message) *error_message = "Cannot open temp chunk for final merge: " + chunk_paths_[i].generic_string();
                return false;
            }
        }
        std::priority_queue<MergeNode, std::vector<MergeNode>, MergeNodeGreater> pq;
        ...
        Writer writer(args_);
        if (!writer.open()) { if (error_message) *error_message = "Cannot open output file: " + args_.output; return false; }
        bool have_prev = false;
        std::string prev_a, prev_b, prev_c;
        while (!pq.empty()) {
            MergeNode node = std::move(const_cast<MergeNode&>(pq.top()));
            ...
            if (have_prev && prev_a == node.rec.a && prev_b == node.rec.b && prev_c == node.rec.c) {
                ...
                ++unique_total;
                if (args_.limit == 0 || stats.written.load(std::memory_order_relaxed) < args_.limit) {
                    TripleView view = node.rec.view();
                    writer.write(view, node.rec.file, node.rec.line, node.rec.source_line, "");
        ...
        return writer.close_and_commit(error_message);
    }

    void write_summary(const Stats& stats, size_t file_count, uint64_t unique_total, size_t final_chunks, size_t merge_passes) {
        if (args_.no_summary || args_.summary.empty()) return;
        std::ofstream s(args_.summary, std::ios::binary | std::ios::trunc);
        if (!s) return;
        double elapsed = elapsed_seconds();
        s << "{\n"
          << "  \"version\": \"" << APP_VERSION << "\",\n"
          << "  \"generated_at\": \"" << now_iso() << "\",\n"
          << "  \"mode\": \"abc-directory-dedupe-external-sort\",\n"
          << "  \"output\": \"" << json_escape(args_.output) << "\",\n"
          << "  \"format\": \"" << args_.format << "\",\n"
          << "  \"parse_mode\": \"" << args_.parse_mode << "\",\n"
          << "  \"threads\": " << args_.threads << ",\n"
          << "  \"chunk_memory_bytes\": " << args_.dedupe_chunk_bytes << ",\n"
          << "  \"merge_fan_in\": " << args_.merge_fan_in << ",\n"
    ...
    Args args_;
    fs::path temp_root_;
    std::vector<fs::path> chunk_paths_;
...
static void print_help() {
    std::cout << R"HELP(rgs-native v5.3.0-external-dedupe
...
独立目录去重 / Standalone directory de-duplication:
  ...
  rgs --dedupe-only -p ./data -o deduped.csv --format csv --memory 512M
...
参数 / Options:
  ...
  -o, --output FILE           输出文件，默认 rg_abc_results.csv
      ...
      --memory SIZE           dedupe 模式内存上限，默认 512M，超过就落盘外部排序
      --temp-dir DIR          dedupe 模式临时 chunk 目录
      --merge-fan-in N        dedupe 模式每轮最多合并 chunk 数，默认 256
...
说明:
  ...
  dedupe 模式：读取目录全部文件 -> 分块排序落盘 -> 外部归并排序 -> 按 A -> B -> C 去重 -> 写出。不会把全部记录放进内存。
)HELP";
}
...
static bool parse_dedupe_args(int argc, char** argv, int start, Args& args, std::string* err) {
    ...
    for (int i = start; i < argc; ++i) {
        ...
        else if (a == "--memory" || a == "--dedupe-memory" || a == "--chunk-size") args.dedupe_chunk_bytes = parse_memory_size_or_mb(need(a.c_str()));
        else if (a == "--temp-dir") args.temp_dir = need(a.c_str());
        else if (a == "--merge-fan-in") args.merge_fan_in = static_cast<size_t>(std::stoull(need(a.c_str())));
        else if (a == "--limit") args.limit = static_cast<size_t>(std::stoull(need(a.c_str())));
        else if (a == "--threads") args.threads = static_cast<unsigned>(std::stoul(need(a.c_str())));
        else if (a == "--summary") { args.summary = need(a.c_str()); args.no_summary = false; }
        else if (a == "--no-summary") { args.summary.clear(); args.no_summary = true; }
...
static int run_interactive_dedupe() {
    ...
    std::cout << "说明：该模式不需要关键词，不走检索 CLI，也不走 GUI。\n"
              << "会读取目录下所有匹配文件，分块排序落盘，再外部归并，按 A -> B -> C 去重。\n" << std::flush;
    ...
    std::string memory_cap = prompt_line("去重内存上限，超过就落盘排序，例如 256M/512M/1G", "512M");
    if (!memory_cap.empty()) args.dedupe_chunk_bytes = parse_memory_size_or_mb(memory_cap);

    args.temp_dir = prompt_line("临时目录，空=输出目录或系统临时目录", "");

    std::string fanin = prompt_line("每轮最多合并 chunk 数 merge-fan-in", "256");
    try { args.merge_fan_in = static_cast<size_t>(std::stoull(fanin)); } catch (...) { args.merge_fan_in = 256; }
